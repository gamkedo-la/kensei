...
public class TakuanSohoDialogueTriggerPV : DialogueTrigger
{
    public GameObject player;
    public GameObject rakusu;
    private bool tookRakusu;
    ...
    public override void OnTriggerEnter2D(Collider2D collider)
    {
        if (collider.CompareTag("Player"))
        {
            ...
            if (GameDictionary.choiceDictionary["Monk Path"] && !tookRakusu)
            {
                //pick which Dialogue to run
    ...
    {
        player.GetComponent<PlayerController>().AddItem(rakusu);
